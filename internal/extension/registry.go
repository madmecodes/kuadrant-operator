...
// for policies targeting the given gateway.
...
func (m *RegisteredDataMutator[TResource]) mutateWasmConfig(wasmConfig *wasm.Config, targetRefs []machinery.PolicyTargetReference) error {
	...
	for _, policyID := range policyIDs {
		...
		for _, entry := range responseEntries {
			...
			case extpb.ActionType_ACTION_TYPE_ADD_HEADERS:
				onReplyActions = append(onReplyActions, wasm.TypedAction{
					Type:                 "headers",
					Predicate:            joinPredicates(entry.Predicates),
					Terminal:             false,
					Target:               "response",
		...
		// Collect AllowAction deny entries (must be nested inside gRPC actions' onReply
		// because the wasm-shim only supports gRPC typed actions as top-level actions)
		...
		// Build request-phase TypedActions (only gRPC actions can be top-level)
		...
		for _, entry := range requestEntries {
			...
			case extpb.ActionType_ACTION_TYPE_GRPC_METHOD:
				...
				// Look up MessageTemplate from the upstream entry
				...
				// Prepend allow-based denies, then intention deny, then response actions
				ta.OnReply = append(ta.OnReply, allowDenyActions...)
				if entry.Intention != "" {
					intentionDeny := wasm.TypedAction{
						Type:                 "deny",
						Predicate:            fmt.Sprintf("!(%s)", entry.Intention),
						Terminal:             true,
						DenyWith:             "DenyResponse{status: 403u}",
					...
					ta.OnReply = append(ta.OnReply, intentionDeny)
				}
		...
		// Append typed actions to each action set
...
// joinPredicates joins a list of CEL predicates into a single predicate string.
