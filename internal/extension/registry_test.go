...
func TestPipelineActionStore_PredicatesPreserved(t *testing.T) {
	store := NewRegisteredDataStore()
	policy := testResourceID("ThreatPolicy", "default", "my-policy")
	...
	store.AppendPipelineActions(policy, PipelinePhaseRequest, []PipelineActionEntry{
		{
			ActionType: extpb.ActionType_ACTION_TYPE_GRPC_METHOD,
			Predicates: []string{"request.headers['check'] == '1'", "request.method == 'GET'"},
			Method:     "checkThreat",
			Intention:  "resp.ok",
		},
	})
	...
	actions := store.GetPipelineActions(policy, PipelinePhaseRequest)
	if len(actions[0].Predicates) != 2 {
		t.Fatalf("Expected 2 predicates, got %d", len(actions[0].Predicates))
	}
	if actions[0].Predicates[0] != "request.headers['check'] == '1'" {
		t.Errorf("First predicate = %q, unexpected", actions[0].Predicates[0])
	}
}
...
func TestMutateWasmConfig_TranslatesPipelineActions(t *testing.T) {
	...
	store.AppendPipelineActions(policyID, PipelinePhaseRequest, []PipelineActionEntry{
		...
		{ActionType: extpb.ActionType_ACTION_TYPE_GRPC_METHOD, Method: "assess-threat", Var: "threatResponse", Intention: "threatResponse.threat_level < 5", Predicates: []string{`"x-assess-threat" in request.headers`}},
	})
	...
	// Only gRPC actions are top-level; allow deny is folded into gRPC onReply
	if len(typed) != 1 {
		t.Fatalf("Expected 1 TypedAction (grpc only), got %d", len(typed))
	}

	// gRPC action
	if typed[0].Type != "grpc" {
		t.Errorf("Expected typed[0] type 'grpc', got %q", typed[0].Type)
	}
	if typed[0].Predicate != `"x-assess-threat" in request.headers` {
		t.Errorf("Expected typed[0] predicate, got %q", typed[0].Predicate)
	}
	if typed[0].Var != "threatResponse" {
		t.Errorf("Expected typed[0] var 'threatResponse', got %q", typed[0].Var)
	}
	if typed[0].Service == "" {
		t.Error("Expected typed[0] service to be set")
	}
	if typed[0].MessageBuilder != "threat.v1.Request{path: request.path}" {
		t.Errorf("Expected typed[0] messageBuilder, got %q", typed[0].MessageBuilder)
	}
	if typed[0].Terminal {
		t.Error("Expected typed[0] to NOT be terminal")
	}

	// onReply: allow deny + intention deny + add_headers
	if len(typed[0].OnReply) != 3 {
		t.Fatalf("Expected 3 onReply actions, got %d", len(typed[0].OnReply))
	}
	// First onReply: allow → deny (negated intention)
	if typed[0].OnReply[0].Type != "deny" {
		t.Errorf("Expected onReply[0] type 'deny', got %q", typed[0].OnReply[0].Type)
	}
	if typed[0].OnReply[0].Predicate != `!(request.url_path != "/blocked")` {
		t.Errorf("Expected onReply[0] predicate, got %q", typed[0].OnReply[0].Predicate)
	}
	if !typed[0].OnReply[0].Terminal {
		t.Error("Expected onReply[0] to be terminal")
	}
	if typed[0].OnReply[0].DenyWith != "DenyResponse{status: 403u}" {
		t.Errorf("Expected onReply[0] denyWith, got %q", typed[0].OnReply[0].DenyWith)
	}
	// Second onReply: intention deny
	if typed[0].OnReply[1].Type != "deny" {
		t.Errorf("Expected onReply[1] type 'deny', got %q", typed[0].OnReply[1].Type)
	}
	if typed[0].OnReply[1].Predicate != "!(threatResponse.threat_level < 5)" {
		t.Errorf("Expected onReply[1] predicate, got %q", typed[0].OnReply[1].Predicate)
	}
	// Third onReply: add headers
	if typed[0].OnReply[2].Type != "headers" {
		t.Errorf("Expected onReply[2] type 'headers', got %q", typed[0].OnReply[2].Type)
	}
	if typed[0].OnReply[2].Headers != `{"x-checked": "true"}` {
		t.Errorf("Expected onReply[2] headers, got %q", typed[0].OnReply[2].Headers)
	}
	if typed[0].OnReply[2].Target != "response" {
		t.Errorf("Expected onReply[2] target 'response', got %q", typed[0].OnReply[2].Target)
	}

	// All typed actions should have source policy locator
	expectedLocator := "ThreatPolicy/default/my-threat"
	if len(typed[0].SourcePolicyLocators) != 1 || typed[0].SourcePolicyLocators[0] != expectedLocator {
		t.Errorf("typed[0]: expected source locator %q, got %v", expectedLocator, typed[0].SourcePolicyLocators)
	}
}
...
func TestMutateWasmConfig_PipelineActionsAppendToMultipleActionSets(t *testing.T) {
	...
	// TypedActions should be appended to BOTH action sets (gRPC with allow deny in onReply)
	for i, as := range wasmConfig.ActionSets {
		...
		// Allow deny should be first in onReply
		if len(as.TypedActions[0].OnReply) < 1 || as.TypedActions[0].OnReply[0].Type != "deny" {
			t.Errorf("ActionSet[%d]: expected allow deny as first onReply action", i)
		}
	}
}
...
func TestApplyWasmConfigMutators_CreatesActionSetsFromTopology(t *testing.T) {
	...
	*GlobalMutatorRegistry = MutatorRegistry{}
	...
	// Only gRPC typed actions are top-level; allow deny is nested in gRPC onReply
	if len(as.TypedActions) != 1 {
	...
	// Allow deny should be first in onReply
	if len(as.TypedActions[0].OnReply) < 1 || as.TypedActions[0].OnReply[0].Type != "deny" {
		t.Fatalf("Expected allow deny as first onReply action")
	}
...
func TestApplyWasmConfigMutators_ExistingActionSetsPreserved(t *testing.T) {
	...
	*GlobalMutatorRegistry = MutatorRegistry{}
	...
	// Allow deny should be first in onReply
	if len(typed[0].OnReply) < 1 || typed[0].OnReply[0].Type != "deny" {
		t.Errorf("Expected allow deny as first onReply action")
	}
}
...
func TestApplyWasmConfigMutators_RouteTargetedPipelineActions(t *testing.T) {
	...
	*GlobalMutatorRegistry = MutatorRegistry{}
	...
	if typed[0].Type != "grpc" {
		t.Errorf("Expected typed action to be grpc, got %s", typed[0].Type)
	}
	if typed[0].Service == "" {
		t.Error("Expected grpc typed action to have service set")
	}
	// Allow deny should be first in onReply, then intention deny
	if len(typed[0].OnReply) < 2 {
		t.Fatalf("Expected at least 2 onReply actions, got %d", len(typed[0].OnReply))
	}
	if typed[0].OnReply[0].Type != "deny" {
		t.Errorf("Expected first onReply to be deny (from allow), got %s", typed[0].OnReply[0].Type)
	}
	if typed[0].OnReply[0].Predicate != `!(request.url_path != "/blocked")` {
		t.Errorf("Expected allow deny predicate, got %q", typed[0].OnReply[0].Predicate)
	}
	if typed[0].OnReply[1].Type != "deny" {
		t.Errorf("Expected second onReply to be deny (from intention), got %s", typed[0].OnReply[1].Type)
	}
}
